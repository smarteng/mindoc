...
func RegisterDataBase() {
	...
	if strings.EqualFold(adapter, "mysql") {
		...
		sdn := "%s:%s@tcp(%s:%s)/%s?charset=utf8mb4&parseTime=true&loc=%s"
		dataSource := fmt.Sprintf(sdn, username, password, host, port, database, url.QueryEscape(timezone))
